...
public class BattleEnemyStatusScript : MonoBehaviour
{
    public enum Status {
        HP,
        MP,
    ...
    //　パーティーステータス
    [SerializeField]
    private EnemyPartyStatus EnemypartyStatus = null;
    ...
    public void DisplayStatus() {
        ...
        for (int i = 0; i < 4; i++) {
            //characterPanelTransform = transform.Find("EnemyCharacterPanel" + i);
            if (i < EnemypartyStatus.GetEnemyStatus().Count) {
                member = EnemypartyStatus.GetEnemyStatus()[i];
